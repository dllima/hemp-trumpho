...
export function criarPartida(nomesJogadores, cartasDisponiveis) {
    ...
    const jogadores = nomesJogadores.map((nome, i) => ({
        id: `j${i}`,
        nome,
        cartas: embaralhadas.slice(i * cartasPorJogador, (i + 1) * cartasPorJogador)
    }));
    return {
        jogadores,
        monteEmpate: [],
...
export function escolherAtributo(estado, atributo) {
    ...
    let novoMonte = [];
    let novosJogadores = [...estado.jogadores];
    if (resultado.monte) {
...
export function obterCartaTopo(jogador) {
